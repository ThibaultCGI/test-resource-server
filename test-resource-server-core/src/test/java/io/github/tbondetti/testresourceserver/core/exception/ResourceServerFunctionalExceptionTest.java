...
class ResourceServerFunctionalExceptionTest {

    @Test
    void constructorOk() {
        final String message = "message";
        final ResourceServerErrorCode code = PRODUIT_NOM_REQUIRED;
        final ResourceServerFunctionalException exception1 = new ResourceServerFunctionalException(code, message);

        assertSame(code, exception1.getCode());
        assertSame(message, exception1.getMessage());

        final ResourceServerFunctionalException exception2 = new ResourceServerFunctionalException(code, message, exception1);
